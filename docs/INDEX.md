...
| **[HOSTS_POR_USUARIO.md](HOSTS_POR_USUARIO.md)** | Quais hosts do cliente cada login de portal enxerga — por host (`UsuarioAcesso`) ou por função (`UsuarioFuncao`) |
...
|------|-------|--------------|
| 17/09/2026 | Arquivos/Firmware: **comandos de download de roteador Huawei VRP** no link de compartilhamento — o bloco Huawei só falava o dialeto de OLT (`load file tftp <ip> <arquivo>`), que um NE8000/NE40E/CE não entende; sobrava colar a URL HTTP dentro de um `tftp ... get`, que também não baixa. Agora o backend devolve os comandos em grupos (`huawei_grupos`): roteador/switch VRP com `tftp <ip> get <arquivo>` e `ftp <ip>` (mais a variante `vpn-instance __LOCAL_OAM_VPN__` de quem baixa pela porta de gerência, e o `get` separado para rodar no prompt `ftp>`), e a OLT MA5800/MA5600 com o `load file` de sempre. Arquivo acima de 32 MB mostra aviso no comando TFTP (65535 blocos de 512 bytes é o teto do protocolo; os `.cc` de NE8000 têm ~1,1 GB, então o caminho é FTP) | README.md |
